...
const OrderPage = () => {
    ...
    useEffect(() => {
        ...
        pb.collection('orders')
            .getFirstListItem(`payment_reference = "${reference}"`, { requestKey: `order-page-${reference}` })
            .then(async (o) => {
                setOrder(o);
                try {
    ...
    const meta = {
        paid: { icon: CheckCircle2, tone: 'text-[hsl(var(--gold))]', title: 'Payment confirmed', text: 'Your preorder is confirmed. A receipt has been sent to your email.' },
        pending: { icon: Package, tone: 'text-[hsl(var(--gold))]', title: 'Preorder recorded', text: 'Your preorder is recorded as pending. You will be notified when payment opens or your order ships.' },
        failed: { icon: XCircle, tone: 'text-[hsl(var(--primary))]', title: 'Payment not completed', text: 'Your payment did not complete. You can retry from your dashboard.' },
                ...
                {meta ? (
                    <div className="border border-border bg-[hsl(0_0%_6%)] p-8 md:p-10">
                            ...
                            <div className="mt-8 border-t border-border pt-6">
                                <div className="grid gap-4 sm:grid-cols-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm capitalize">{order.payment_status}</p>
                                    </div>
                                    <div>
                        ...
                        <div className="mt-8 flex flex-wrap gap-4">
                            ...
                            <Link to="/book" className="border border-border px-6 py-3 text-[0.66rem] uppercase tracking-[0.22em] text-muted-foreground hover:text-foreground">
                                Back to editions
                            </Link>
                        </div>
                    </div>
